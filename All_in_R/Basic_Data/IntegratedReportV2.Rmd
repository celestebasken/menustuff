...
dining_hall_name <- "C3"
dining_hall_label <- "Cafe 3"

# Scenario 3 setting: target cost reduction
scenario3_cost_reduction_target <- 0.07
...
build_base_constraints <- function(opt_df, lower_multiplier = 0.5, upper_multiplier = 1.5) {
...
export_scenario_deliverable <- function(
  ...
  save_plot(
    report_objs$plots[[scenario_name]]$category_share_change,
    paste0("plot_category_share_change_", scenario_name, ".png"),
    output_dir = output_dir,
    width = 8,
    height = 5
  )
...
# =========================================================
# RUN FULL ANALYSIS FOR ONE DINING HALL
# =========================================================

run_full_dining_hall_analysis <- function(
    data_dir,
    dining_hall_name,
    dining_hall_label,
    output_root = "scenario_outputs",
  ...
  output_dir <- file.path(output_root, dining_hall_name)
  dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

  write_csv(opt_df, file.path(output_dir, paste0("optimization_data_", dining_hall_name, ".csv")))
  write_csv(report_objs$scenario_summary, file.path(output_dir, paste0("scenario_summary_", dining_hall_name, ".csv")))
  write_csv(report_objs$category_frequency, file.path(output_dir, paste0("category_frequency_", dining_hall_name, ".csv")))
  write_csv(report_objs$category_sustainability, file.path(output_dir, paste0("category_sustainability_", dining_hall_name, ".csv")))

  exported <- purrr::map(
    names(scenario_list),
    ~ export_scenario_deliverable(
      report_objs = report_objs,
      scenario_name = .x,
      output_dir = output_dir,
      dining_name = dining_hall_label
    )
  )
  names(exported) <- names(scenario_list)

  list(
    optimization_data = opt_df,
...
results <- run_full_dining_hall_analysis(
  data_dir = data_dir,
  dining_hall_name = dining_hall_name,
...
# View combined comparison table
print(results$report_objs$scenario_summary)

# Example: show narrative for Scenario 3
scenario3_name <- paste0("s3_", dining_hall_name)
if (scenario3_name %in% names(results$exported)) {
  cat(results$exported[[scenario3_name]]$narrative_text, "\n")
}

# Example: display Scenario 3 plots in session
if (scenario3_name %in% names(results$report_objs$plots)) {
  print(results$report_objs$plots[[scenario3_name]]$category_frequency)
  print(results$report_objs$plots[[scenario3_name]]$category_share_change)
  print(results$report_objs$plots[[scenario3_name]]$sus_vs_conv_spend)
}
```
