...
optimization_hypothetical <- read.csv("optimization_data_hypotheticals.csv", stringsAsFactors = FALSE)
```
...
x0 <- optimization_hypothetical$Menu_freq_f25
n  <- nrow(optimization_hypothetical)
...
price <- optimization_hypothetical$Avg_price_lb
ghg   <- optimization_hypothetical$Conventional_GHG_per_lb

sus_flag <- ifelse(tolower(optimization_hypothetical$Default_sus) %in% c("yes","y","true","1","Yes"), 1, 0)
...
# ============================================================
optimization_hypothetical$freq_opt_s2 <- x2_final
optimization_hypothetical$delta_s2 <- optimization_hypothetical$freq_opt_s2 - optimization_hypothetical$Menu_freq_f25

```
```{r}

optimization_hypothetical <- optimization_hypothetical %>%
  mutate(
    freq_baseline = Menu_freq_f25,
    delta_s2 = freq_opt_s2 - Menu_freq_f25
  )

category_totals_s2 <- optimization_hypothetical %>%
  group_by(Category) %>%
  summarise(
...
write.csv(optimization_hypothetical, file = "optimization_including_scenario2b.csv", row.names = FALSE)
